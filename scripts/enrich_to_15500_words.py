"""
Script to expand all chapters with comprehensive technical depth to achieve >= 15,200 words.
"""

# Let's enrich ch2_gen.py, ch4_gen.py, ch5_gen.py, ch6_gen.py, ch7_gen.py
...
def generate_chapter_2(add_p, add_ch_heading, add_sec_heading, add_subsec_heading, add_code, add_table, add_fig, fig_dir):
    ...
    add_p("In particular, modern microservice architectures require distributed tracing across service boundaries. Traditional log collection misses the inter-process call chains that modern cloud-native attacks exploit. By integrating OCSF and graph traversal, AiSOC reconstructs the full distributed provenance of multi-tier cloud interactions.")
    ...
    add_p("Furthermore, OCSF enforces an explicit observable entity model. An event payload includes structured arrays of IP addresses, domain names, user accounts, and process hashes, allowing stream processors to extract indicators of compromise without executing expensive regular expression queries.")
